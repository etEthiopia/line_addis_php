		...
		window.onYouTubeIframeAPIReady = function() {
			...
			$('.video-slide').each(function(i) {
				$('.video-container')
					.eq(i)
					.append(
						'<div id ="mbr-video-' + i + '" class="mbr-background-video" data-video-num="' + i + '"></div>'
					)
...
(function(b) {
	...
	function m(a) {
		var c = b(a.target);
		c[0]._masonryProgress ||
			((c[0]._masonryProgress = !0),
			'undefined' !== typeof b.fn.masonry &&
				c.outerFind('.mbr-gallery').each(function() {
					var a = b(this).find('.mbr-gallery-row');
					a.one('layoutComplete', function() {
						c[0]._masonryProgress = !1;
					});
	...
	function q(a, c, b, d, f) {
		var g = a.find('img');
		if (g[0].complete && 50 < g[0].naturalWidth && 50 < g[0].naturalHeight) r(g, a, c, b, d, f);
		else
			g.one('load', function() {
				r(g, a, c, b, d, f);
			});
	}
	function r(a, c, b, d, f, g) {
		var l = a[0].naturalWidth,
			p = a[0].naturalHeight;
		d = d / b > l / p ? (b - 2 * g) * l / p : d - 2 * g;
		d = d >= l ? l : d;
		b = (b - d * p / l) / 2;
		a.css({
			width: parseInt(d),
	...
	function t() {
		...
		if (h) {
			var e,
				d = !1;
			h.find('.modal-dialog .carousel-item.carousel-item-next, .modal-dialog .carousel-item.carousel-item-prev')
				.length
				? (e = h.find(
						'.modal-dialog .carousel-item.carousel-item-next, .modal-dialog .carousel-item.carousel-item-prev'
					))
				: ((e = h.find('.modal-dialog .carousel-item.active')), (d = !0));
			e[0].classList.contains('video-container') ? e.css({ top: 0, height: c - 0 - 20 }) : q(e, c, a, 0, 10);
			clearTimeout(u);
			u = setTimeout(z, 200, h, a, c, 0, 10, d);
		}
	}
	var k = b('html').hasClass('is-builder');
	if (!k) {
		...
		window.onYouTubeIframeAPIReady = function() {
			...
			b('.video-slide').each(function(a) {
				b('.video-container')
					.eq(a)
					.append(
						'<div id ="mbr-video-' + a + '" class="mbr-background-video" data-video-num="' + a + '"></div>'
					)
					.append('<div class="item-overlay"></div>');
				b(this).attr('data-video-num', a);
				if (-1 !== b(this).attr('data-video-url').indexOf('vimeo.com')) {
					var e = { id: b(this).attr('data-video-url'), width: '100%', height: '100%', loop: !0 };
					a = new Vimeo.Player('mbr-video-' + a, e);
					...
					d =
						'false' === d
							? !1
							: (d = /(?:\?v=|\/embed\/|\.be\/)([-a-z0-9_]+)/i.exec(d) || /^([-a-z0-9_]+)$/i.exec(d))
								? d[1]
								: !1;
					a = new e('mbr-video-' + a, {
						height: '100%',
	...
	b(document).on('add.cards', function(a) {
		var c = b(a.target);
		c.on('click', '.mbr-gallery-filter li', function(a) {
			a.preventDefault();
			var d = b(this).closest('li');
			d.parent().find('li').removeClass('active');
			d.addClass('active');
			a = d.closest('section').find('.mbr-gallery-row');
			var f = b(this)[0].textContent.trim();
			c.find('.mbr-gallery-item').each(function(a, c) {
				var e = b(this),
					h = e.attr('data-tags').split(',').map(function(a) {
						return a.trim();
					});
				-1 !== b.inArray(f, h) || d.hasClass('mbr-gallery-filter-all')
					? e.removeClass('mbr-gallery-item__hided')
					: (e.addClass('mbr-gallery-item__hided'), e.css('left', '300px'));
			});
			a.closest('.mbr-gallery-row').trigger('filter');
		});
	});
	if (k)
		b(document).on('changeButtonColor.cards', function(a) {
			var c = b(a.target);
			if (
				0 < c.find('.mbr-gallery-filter').length &&
				b(a.target).find('.mbr-gallery-filter').hasClass('gallery-filter-active')
			) {
				var e = (c.find('.mbr-gallery-filter .mbr-gallery-filter-all').find('a').attr('class') || '')
					.replace(/(^|\s)active(\s|$)/, ' ')
					.trim();
				c.find('.mbr-gallery-filter ul li:not(.mbr-gallery-filter-all) a').attr('class', e);
			}
			m(a);
		});
	b(document).on('add.cards changeParameter.cards', function(a) {
		...
		c.find('.mbr-gallery-item').each(function(a) {
			(b(this).attr('data-tags') || '').trim().split(',').map(function(a) {
				a = a.trim();
	...
	k.on('show.bs.modal', function(a) {
		...
		x = setTimeout(function() {
			...
			0 < c.length &&
				...
				((c = n[+c.attr('data-video-num')]), c.playVideo ? c.playVideo() : c.play());
		}, 500);
		h = b(a.target);
		t();
	});
	k.on('slide.bs.carousel', function(a) {
		...
		0 < a.length && ((a = n[+a.attr('data-video-num')]), a.pauseVideo ? a.pauseVideo() : a.pause());
	});
	b(window).on('resize load', t);
	k.on('slid.bs.carousel', function(a) {
		a = b(a.target).find('.carousel-item.active .mbr-background-video');
		0 < a.length && ((a = n[+a.attr('data-video-num')]), a.playVideo ? a.playVideo() : a.play());
	});
